...
type Identifier = string * LspRange
type Code = string * LspRange
let inline (|Ident|) ((id, _): Identifier) = id
...
type ParserSpec =
    {
        Header: Code
        Tokens: (Identifier * Identifier option) list
...
let mkItem0 (prodIdx, dotIdx) : Item0 =
    (uint32 prodIdx <<< 16) ||| uint32 dotIdx
...
let (|PTerminal|PNonTerminal|) x =
    if x < 0 then PTerminal(-x - 1) else PNonTerminal x
...
let (|GotoItemIdx|) (i64: uint64) =
    int32 ((i64 >>> 32) &&& 0xFFFFFFFFUL), int32 (i64 &&& 0xFFFFFFFFUL)
...
type NonTerminalTable(nonTerminals: NonTerminal list) =
    let nonterminalsWithIdxs =
        List.mapi (fun (i: NonTerminalIndex) n -> (i, n)) nonTerminals
...
type ProductionTable(ntTab: NonTerminalTable, termTab: TerminalTable, nonTerminals: string list, prods: Production list) =
    ...
    let b =
        Array.ofList (List.map (fun (_, Production(nt, _, _, _)) -> ntTab.ToIndex nt) prodsWithIdxs)

    let c =
        Array.ofList (List.map (fun (_, Production(_, prec, _, _)) -> prec) prodsWithIdxs)

    let productions =
        nonTerminals
        |> List.map (fun nt ->
            (ntTab.ToIndex nt, List.choose (fun (i, Production(nt2, _, _, _)) -> if nt2 = nt then Some i else None) prodsWithIdxs))
        |> CreateDictionary
...
type PropagateTable() =
    ...
    member table.Add(a, b) =
        if not (t.ContainsKey(a)) then
            t.[a] <- HashSet<KernelItemIndex>(HashIdentity.Structural)
...
let CompilerLalrParserSpec logf (spec: ProcessedParserSpec) : CompiledSpec =
    ...
    let terminals =
        spec.Terminals
        @ [ (dummyLookahead, dummyPrec); (endOfInputTerminal, dummyPrec) ]
                    ...
                    for nonTerm in ntTab.Indexes do
                        yield
                            (PNonTerminal nonTerm,
                             List.foldBack
                                 (fun prodIdx acc ->
                                     match prodTab.Symbol prodIdx 0 with
    ...
    let prec_of_item0 item0 =
        prodTab.Precedence(prodIdx_of_item0 item0)

    let ntIdx_of_item0 item0 =
        prodTab.NonTerminal(prodIdx_of_item0 item0)
    ...
    let fakeStartNonTerminalsSet =
        Set.ofList (fakeStartNonTerminals |> List.map ntTab.ToIndex)

    let IsStartItem item0 =
        fakeStartNonTerminalsSet.Contains(ntIdx_of_item0 item0)
    ...
    let OutputItem0 os item0 =
        fprintf
            os
            "    %s -> %a . %a"
            (ntTab.OfIndex(ntIdx_of_item0 item0)) (* outputPrecInfo precInfo *)
            OutputSyms
            (lsyms_of_item0 item0)
            OutputSyms
    ...
    let OutputFirstMap os m =
        Map.iter
            (fun x y ->
                fprintf os "first '%a' = " OutputSym x
    ...
    let OutputActions os m =
        Array.iteri
            (fun i (prec, action) ->
                let term = termTab.OfIndex i in fprintfn os "    action '%s' (%a): %a" term outputPrecInfo prec OutputAction action)
            m

    let OutputActionTable os m =
        Array.iteri
            (fun i n ->
                fprintfn os "state %d:" i
    ...
    let OutputGotos os m =
        Array.iteri
            (fun ntIdx s ->
                let nonterm = ntTab.OfIndex ntIdx in

                match s with
                | Some st -> fprintfn os "    goto %s: %d" nonterm st
                | None -> ())
            m
    ...
    let ComputeClosure0NonTerminal =
        Memoize(fun nt ->
            let seed =
                (List.foldBack (prodIdx_to_item0 >> Set.add) prodTab.Productions.[nt] Set.empty)
    ...
    let startItems =
        List.mapi (fun i _ -> prodIdx_to_item0 (startNonTerminalIdx_to_prodIdx i)) fakeStartNonTerminals
    ...
    let startKernelItemIdxs =
        List.map2 (fun a b -> KernelItemIdx(a, b)) startKernelIdxs startItems
                        ...
                        | Some gotoKernelIdx ->
                            ...
                            for lookaheadToken in lookaheadTokens do
                                if lookaheadToken = dummyLookaheadIdx then
                                    propagate.Add(item0Idx, gotoItemIdx) |> ignore
                                else
    ...
    let actionTable, immediateActionTable =
        ...
        Array.ofList (List.map fst actionInfo), Array.ofList (List.map snd actionInfo)
    ...
    let gotoTable =
        let gotos kernelIdx =
            Array.ofList (List.map (fun nt -> gotoKernel (GotoItemIdx(kernelIdx, PNonTerminal nt))) ntTab.Indexes)
